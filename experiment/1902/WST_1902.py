...
def make_block():
    diff_trials = np.random.permutation(np.tile([0,1], ntrials//2)) #0101... for ntrials
    feat_change = np.random.permutation(np.tile([0,1],sum(diff_trials)//2)) #0101... for n change trials
    
    count=0
    diff_trials_feat = []
    for i in diff_trials:
        if i == 0:
            diff_trials_feat.append(2)
        else:
            diff_trials_feat.append(feat_change[count])
            count+=1
    diff_trials_feat = np.array(diff_trials_feat)

    num_stim = np.random.permutation(np.tile([1,2,3,4], ntrials//4)) #1,2,3,4,5 for n trials, assumes divisible by 5
    
    return num_stim, diff_trials, diff_trials_feat
...
def make_orient_trial(diff_trials, diff_trials_feat, num_stim, itrial):
    ...
    stim_color = np.zeros((num_stim[itrial],3))
    ...
    if diff_trials[itrial] == 0: #same trials
        ...
        other_color = np.setdiff1d(color_array_idx,stim_color_idx[0])
        
        # only one feature changes
        if diff_trials_feat[itrial] == 0: # color change
            # choose color that isn't same 
            test_color_idx = np.random.choice(other_color,size=1)
            test_color = rgb_table[int(test_color_idx)]
            #orient stays the same
            test_orient = orient_table[stim_orient_idx[0]] 
        else: # orient change
            # choose orient that isn't same 
            test_orient_idx = np.random.choice(other_orient,size=1)
            test_orient = orient_table[int(test_orient_idx)]
            # color stays the same
            test_color = rgb_table[stim_color_idx[0]] 

    trial = {
                'set_size': num_stim[itrial],
                'trial_type': diff_trials[itrial],
                'change_feat':diff_trials_feat[itrial],
                'trial_'
                'cresp': cresp,
                'locations': locs,
...
def display_orient_trial(trial, trial_idx,itrial, iblock, win, fix,orient_bar,circle,white_track,question,experiment_info,num_prac=None,tracker=None):
    ...
    if itrial == 0:
        text_screen(text='Press S to begin', input_keys = 's',win=win)
    ...
    for istim in range(trial['set_size']):
        ...
        circle.fillColor = trial['stim_color'][istim]
        circle.draw()
        orient_bar.draw()
    ...
    for istim in range(5-trial['set_size']):
        ...
        circle.fillColor = trial['dist_color']
        circle.draw()
        orient_bar.draw()
    ...
    circle.fillColor = trial['test_color']
    circle.draw()
    orient_bar.draw()
    
    if question['EEG']=='Y':
        if num_prac is None:
            if trial['change_feat'] == 0: # color change
                code = 31
            elif trial['change_feat'] == 1: # orient change
                code = 32
            else: # no change
                code = 33
            ...
            if question['Eyetracking'] == 'Y':
                tracker.send_message(sync + str(code))
    win.flip()
...
def run_exp(): 
    ...
    if question['Practice'] == 'Y':
        ...
        while prac_cont == ['y']:
            ...
            for itrial in range(12):
                ...
                trial = make_orient_trial(diff_trials=diff_trials, num_stim=num_stim,itrial=itrial) #make trial info
    ...
    for iblock in range(nblocks):
        num_stim, diff_trials,diff_trials_feat = make_block()
        block_acc_num = 0
        block_acc = 0
        ...
        for itrial in range(ntrials):
            ...
            ###EXPERIMENT###
            trial = make_orient_trial(diff_trials=diff_trials, diff_trials_feat=diff_trials_feat, num_stim=num_stim, itrial=itrial) #make trial info
